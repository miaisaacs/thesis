...
denv1_protein_regions <- denv1_protein_regions |> 
  mutate(Name = ifelse(!is.na(product) & str_trim(product) != "", product, Name)) |> 
  select(Name, Minimum, Maximum)

view(denv1_protein_regions)
```
...
### clean data to limit to only necessary proteins
```{r}
target_proteins <- c(
  "capsid protein C", 
  "protein pr", 
...
protein_locations <- denv1_protein_regions |>
  filter(Name %in% target_proteins) |>
  arrange(Minimum)

protein_locations <- protein_locations |>
  mutate(
    Protein = case_when(
      str_detect(Name, "capsid") ~ "C",
      str_detect(Name, "protein pr") ~ "pr",
      str_detect(Name, "glycoprotein M") ~ "M",
      str_detect(Name, "envelope protein E") ~ "E",
      str_detect(Name, "NS1") ~ "NS1",
      str_detect(Name, "NS2A") ~ "NS2A",
      str_detect(Name, "NS2B") ~ "NS2B",
      str_detect(Name, "NS3") ~ "NS3",
      str_detect(Name, "NS4A") ~ "NS4A",
      str_detect(Name, "NS4B") ~ "NS4B",
      str_detect(Name, "NS5") ~ "NS5"
    )
  )

view(protein_locations)
```

### match probe start and end positions to protein
```{r}
processed_data <- processed_data |>
  rowwise() |>
  mutate(
    protein = protein_locations$Protein[
      which(start_pos >= protein_locations$Minimum & start_pos <= protein_locations$Maximum)
    ][1]
  ) |>
  ungroup()
...
```{r}
maybe_working <- read_csv("data/help.csv")
