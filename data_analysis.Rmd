...
## making plots foor reactivity across genome for each serotype and timepoint

### set midpoint for easier plotting
```{r}
figure3_df <- final_df_long |> 
  mutate(midpoint = (start + end) / 2) |> 
  filter(hit_yn == 1)

ggplot(figure3_df, aes(x = midpoint, y = fluorescence_log)) +
  geom_point(alpha = 0.7, size = 1.5, color = "blue") +
  facet_grid(serotype ~ timepoint) +  
  labs(
    title = "Fluorescence (log-transformed) Across DENV1 Genome",
    x = "Genome Position (Midpoint of Probe)",
    y = "Fluorescence (log10 scale)"
  ) +
  theme_minimal() +
  theme(
