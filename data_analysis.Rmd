...
## creating VARscore to use in identification of significant reactive epitopes

### fit bimodal distribution using E-M algorithim
```{r}
# fit Gaussian Mixture Model (GMM) to fluorescence values using mclust
gmm <- Mclust(dengueIgG_serotype$fluorescence_norm, G = 2) # G = 2 specifies bimodal distribution

# extract parameters for noise distribution (assume it's the smaller mean)
noise_component <- which.min(gmm$parameters$mean) # index of noise component
mean_noise <- gmm$parameters$mean[noise_component]
sd_noise <- sqrt(gmm$parameters$variance$sigmasq[noise_component])
...
### adjust MFI values
```{r}
# subtract threshold from reactivity values and set negatives to zero
dengueIgG_serotype <- dengueIgG_serotype |> 
  mutate(adjusted_reactivity = pmax(fluorescence_norm - threshold, 0)) # pmax ensures no negative values
